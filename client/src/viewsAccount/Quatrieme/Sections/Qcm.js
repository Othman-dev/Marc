    ...
    const setResult = useCallback((result) => {
		...
		search.forEach((card, index) => {
		    ...
		    card.forEach((course, index) => Array.isArray(course) 
				? card[index]='' : course)
		    if((card.toString().toLowerCase().replace(/,/g , ' ').indexOf('qcm') >= 0)
		    && (card.toString().toLowerCase().replace(/,/g , ' ').indexOf('quatrieme') >= 0))
				{validation.push(1)}
  ...
  useEffect(() => {
    ...
    return (
		...
		<div className='right-side'>
            ...
            <Header logoOff title="Les QCM | Section 4ème" links={getLinks(routes)} handleChange={handleChange}/>
			<div>
						<table>
							<thead>
								<tr>
