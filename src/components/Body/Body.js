	...
	const setResult = useCallback((result) => {
		...
		search.forEach((card, index) => {
			...
			selected.forEach(tag => {
				if (tag.includes('$')) {
					if (card.toString().toLowerCase().replace(/,/g, ' ').indexOf(tag.slice(1).toLowerCase()) !== -1) { validation.push(1) }
					else { validation.push(0) }
				} else {
					card.forEach((item, index) => Array.isArray(item) ? card[index] = '' : item)
					if (card.toString().toLowerCase().replace(/,/g, ' ').indexOf(tag.toLowerCase()) !== -1) { validation.push(1) }
					else { validation.push(0) }
				}
=======
    const { selected } = useContext(HashtagContext);
...
>>>>>>> abcb9fab48e66ea448d07db96a70d8a97adf6797:src/components/Body/Body.js
			})
