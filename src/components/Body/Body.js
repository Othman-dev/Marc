	...
	const setResult = useCallback((result) => {
		...
		search.forEach((card, index) => {
			...
			selected.forEach(tag => {
				if (tag.indexOf('$') === 0) {
					if (card.toString().toLowerCase().replace(/,/g, ' ').indexOf(tag.slice(1).toLowerCase()) >= 0) { validation.push(1) }
					else { validation.push(0) }
				} else {
					if (temp.toString().toLowerCase().replace(/,/g, ' ').indexOf(tag.toLowerCase()) >= 0) { validation.push(1) }
					else { validation.push(0) }
				}
			})
